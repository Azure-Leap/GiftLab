...
const DescriptionPostAboutPaymentWithDescriptionAndPosts = ({clickedPost} : any) => {
  ...
  return (
    <div className='col-span-3 rounded-[32px] row-span-6  flex flex-col gap-5 h-[93vh] overflow-y-scroll relative  ' id='top'>
      {/* Top */}
        <div className='grid grid-cols-2 gap-10 bg-white p-[1rem_2rem_2rem_2rem] rounded-[32px]' id='mainContent'>
            {/* Right */}
            <div className='col-span-1  gap-10 content-center grid'>
              ...
              <div className='grid grid-cols-4 gap-4'>
                {clickedPost.carouselImg.map((img, imgIdx) => (
        ...
        <div className='grid grid-cols-2  rounded-[32px] gap-5'>
                <div className='flex flex-col justify-between bg-white p-5 rounded-[32px] gap-5'>
                  <div className='col-span-1  flex flex-col gap-4  '>
                    ...
                    <div className='flex gap-1 items-center ms-3'>
                      ...
                      <p className='font-semibold text-[14px]'>{clickedPost.limit ? `${clickedPost.members} / ${clickedPost.limit}` : `${clickedPost.members}`} people going</p>
                    </div>
                    {/* Event By */}
                    <div className='flex gap-1 items-center ms-3'>
                        <span className='font-semibold text-[14px]'><BsPerson size={19} /></span>
                        <p className='text-[14px] font-semibold'>{clickedPost.whoPosted}</p>
                    ...
                    <div className='flex gap-1 items-center ms-3'>
                        ...
                        <p className='text-[14px] font-semibold'>{clickedPost.locationSpecific}</p>
                    </div>

                    {/* Duration */}
                    <div className='flex gap-2 items-center ms-3'>
                        <span><FiClock/></span>
                        <p className='text-[14px] font-semibold'>{clickedPost.duration}</p>
                ...
                <AddToCartBtn  bg='bg-[#000]' padX='p-[10px_30px]' outlineColor='outline-[#000]' clickedPost={clickedPost}/>
            ...
            <div className='bg-white rounded-[32px] p-[2rem] '>
                ...
                <div className='flex flex-col gap-2'> 
                    {clickedPost.goals.map((goal, idx) => (
        ...
        <div onClick={()=>{
            ...
            }}  className='flex justify-center rounded-[312px] overflow-hidden bg-white mx-auto border-2 border-black  p-[10px] absolute bottom-[40px] left-[50%] -translate-x-[50%] cursor-pointer'>
            <span ref={el=>chevronIcon=el}><FaChevronDown/></span>
        </div>
